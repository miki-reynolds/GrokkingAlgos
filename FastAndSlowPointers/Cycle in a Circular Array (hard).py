...
In our algorithm, we don’t keep a record of all the numbers that have been evaluated for cycles. We know that all such numbers will not produce a cycle for any other instance as well.
If we can remember all the numbers that have been visited, our algorithm will improve to O(N) as, then, each number will be evaluated for cycles only once. We can keep track of this by creating a separate array, however, in this case, the space complexity of our algorithm will increase to O(N).
"""


def circular_array_loop_exists(array):
    for i in range(len(array)):
        is_forward = array[i] >= 0  # bool value from the chain comparison
        slow, fast = i, i

        # if slow or fast becomes '-1' this means we can't find cycle for this number.
        # loop moves on to another number (array[i])
        while True:
            # move one step for slow pointer
            slow = find_next_index(array, is_forward, slow)
            # move one step for fast pointer
            fast = find_next_index(array, is_forward, fast)

            if fast != -1:  # move another step for fast pointer
                fast = find_next_index(array, is_forward, fast)
...
def find_next_index(array, is_forward, current_index):
    direction = array[current_index] >= 0  # bool value from the chain comparison

    if direction != is_forward:
        return -1  # change in direction, return -1

    next_index = (current_index + array[current_index]) % len(array)  # mod

...
def shortest_window_sort(array):
    low, high = 0, len(array) - 1

    # low = the position of the element that breaks the order when the loop stops.
    while (low < len(array) - 1) and (array[low] <= array[low+1]):
        low += 1

    if low == len(array) - 1:  # the array is sorted.
        return 0

    # high = the position of the element that breaks the order when the loop stops.
    while (high > 0) and (array[high] >= array[high-1]):
        high -= 1

    subarray_min = math.inf
    subarray_max = -math.inf
    for k in range(low, high+1):
        subarray_min = min(subarray_min, array[k])
        subarray_max = max(subarray_max, array[k])

    # extend the subarray to include any number which is bigger than the minimum of
    # the subarray
    while (low > 0) and array[low-1] > subarray_min:
        low -= 1

    # extend the subarray to include any number which is smaller than the maximum of
    # the subarray
    while (high < len(array) - 1) and array[high+1] < subarray_max:
        high += 1
